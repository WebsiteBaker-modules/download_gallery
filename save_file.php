...
if ((isset($_POST['delete_file']) AND $_POST['delete_file'] == 'yes') or (isset($_POST['delete_file2']) AND $_POST['delete_file2'] == 'yes')) {
...
if (isset($_POST['delete_counter']) AND $_POST['delete_counter'] == 'yes') {
...
if(trim($existingfile!='')) {
  ...
  if  ($remotelink == '') {
	...
	//if file is to be deleted nedd to clear the record filename so a new file can be put into place with the existing title,description
...
// Update row
$database->query("UPDATE ".TABLE_PREFIX."mod_download_gallery_files SET title = '$title', link = '$file_link', group_id = '$group', `description` = '$description', active = '$active',
	modified_when = '".time()."', modified_by = '".$admin->get_user_id()."' WHERE file_id = '$file_id' and page_id = '$page_id'");
