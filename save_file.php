...
if($admin->get_post('title') == '' AND $admin->get_post('url') == '') {
        $admin->print_error($MESSAGE['GENERIC_FILL_IN_ALL'], WB_URL.'/modules/download_gallery/modify_file.php?page_id='.$page_id.'&section_id='.$section_id.'&file_id='.$id);
} else {
    ...
    $aTmp = parse_url($old_link);
    $old_link = (isset($aTmp['path']) ? $aTmp['path'] : MEDIA_DIRECTORY.'/'.basename(__DIR__).'/'.basename($old_link));
    if(($existingfile=="") AND ($remotelink=="")) {$existingfile=$old_link;}
...
if(trim($existingfile!='')) {
    ...
    if ($file_link!=''){
        $aTmp = parse_url($file_link);
        $file_link = (isset($aTmp['path']) ? $aTmp['path'] : MEDIA_DIRECTORY.'/'.basename(__DIR__).'/'.basename($file_link));
    }
$released = time();
$filelink = (strpos($file_link, '\\')===0) ? $file['link'] : WB_PATH.str_replace(WB_URL,'',$file_link);
