...
/*
         ...
         http://cms/wb_eurohof_deetz/modules/download_gallery/dlc.php?file=13&id=1410884662
*/
error_reporting(0);
...
$total_num = $database->query("SELECT `title` FROM `".$DG_PREFIX."_files` WHERE `section_id` = '$section_id' AND `active` = '1' AND `file_id` = '$dlFile' AND `title` != ''")->numRows();

// Work-out if we need to add limit code to sql
$limit_sql = ($settings['files_per_page'] != 0) ? " LIMIT 1" : "";
...
$query_files = $database->query("SELECT ".$DG_PREFIX."_files.`title` AS 'file_title', ".$DG_PREFIX."_files.*
                ...
                AND  ".$DG_PREFIX."_files.`file_id` = '$dlFile'
                ".$dlsearch."
...
if($num_files > 0) {
        ...
        $button_placeholders = array(
                ...
                '[THRELEASED]'         => sprintf($input_button_pattern, 1, $ctitle,         $adtitle,         1, $DGTEXT['THRELEASED'],         $DGTEXT['THRELEASED']),
        ...
        while($file = $query_files->fetchRow()) {
                ...
                $filelink = (strpos($file['link'], '\\')===0) ? $file['link'] : WB_PATH.str_replace(WB_URL,'',$file['link']);
                        ...
                        $file_date = gmdate(DATE_FORMAT, $file['modified_when']);
                        $file_time = date(TIME_FORMAT, $unixtime);
                        $release_date= gmdate(DATE_FORMAT, $unixtime);
                        $release_time= gmdate(DATE_FORMAT, $unixtime);
                        ...
                        // update file date in DB if necessary:
                        if (($file['released'] != $unixtime) && ($unixtime > 0)) {

                                //echo "<!-- DEBUG file size probably changed -->\n";
                                $file_id = $file['file_id'];
                                $database->query("UPDATE `".$DG_PREFIX."_files` SET `released` = '$filesize' WHERE `file_id` = '$file_id'");
