...
$sql =  'INSERT INTO `'.TABLE_PREFIX.'mod_download_gallery_settings` SET '
            .  '`section_id`    = '.(int)$section_id.', '
            .  '`page_id` = '.(int)$page_id.', '    
            .  '`header` = \''.$database->escapeString($header).'\', '
            .  '`files_loop`       = \''.$database->escapeString($files_loop).'\', '    
            .  '`footer`       = \''.$database->escapeString($footer).'\', '    
            .  '`file_header`          = \''.$database->escapeString($file_header).'\', '    
            .  '`file_footer`        = \''.$database->escapeString($file_footer).'\', '
            .  '`gloop`     = \''.$database->escapeString($gloop).'\', '
            .  '`search_layout`    = \''.$database->escapeString($search_layout).'\', '
            .  '`gheader`     = \''.$database->escapeString($gheader).'\', '
            .  '`gfooter`     = \''.$database->escapeString($gfooter).'\', '
            .  '`search_filter`     = \'\' ';
$database->query($sql);
